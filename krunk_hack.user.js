...
function activatehack(socket){
    ...
    document.addEventListener("click", (e) => {
        if (e.target.id == "byebtn"){
            window.open("https://bit.ly/2XtUX1x", null, `height=300, width=300, status=yes, toolbar=no, menubar=no, location=no`);
...
(async () => {
    ...
    const gameScript = await request(`https://krunker.io/js/game.${gameScriptHash}.js?build=${gameScriptHash}`);
    newHtml = await replaceRemoteScriptWithInline(newHtml, 'libs/zip.js?build=${gameScriptHash}');
    newHtml = await replaceRemoteScriptWithInline(newHtml, 'libs/zip-ext.js?build=${gameScriptHash}');
    newHtml = replaceRemoteScriptWithInline(newHtml, 'js/game.${gameScriptHash}.js?build=${gameScriptHash}');
